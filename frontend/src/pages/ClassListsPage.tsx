...
const sanitizeNamePart = (value: string) => formatStudentName(String(value || '').replace(/,/g, ' ').trim());
...
export function ClassListsPage() {
  ...
  const splitNameToEditorRow = (name: string, index: number): ClassListEditorRow => {
    ...
    if (!rawName) {
      return { id: `row-${index}`, firstName: '', lastName: '' };
    }
    if (rawName.includes(',')) {
      const [left, right] = rawName.split(',', 2);
      return {
        id: `row-${index}`,
        firstName: sanitizeNamePart(right),
        lastName: sanitizeNamePart(left),
      };
    }
    const parts = sanitizeNamePart(rawName).split(' ').filter(Boolean);
    if (parts.length === 0) {
      return { id: `row-${index}`, firstName: '', lastName: '' };
    }
    if (parts.length === 1) {
      return { id: `row-${index}`, firstName: parts[0], lastName: '' };
    }
    if (parts.length === 2) {
      return {
        id: `row-${index}`,
        ...
        {!isLoading && sortedClassLists.length > 0 && (
          <div className="workspace-card-grid">
            {sortedClassLists.map((classList) => (
              <Fragment key={classList.id ?? `${classList.name}-${classList.created_at}`}>
                <article className="workspace-card">
                  ...
                  <div className="actions-row" style={{ marginTop: 0 }}>
                    <button
                      ...
                      onClick={() => (activeClassListId === classList.id ? closeEditor() : openClassList(classList))}
                    >
                      {activeClassListId === classList.id ? 'Close' : 'Check/Edit'}
                    </button>
                    <button
                      type="button"
                ...
                {activeClassListId === classList.id && activeClassList && (
                  <section className="card stack" style={{ gridColumn: '1 / -1', marginBottom: 0 }}>
                    <div className="panel-title-row">
                      <div>
                    ...
                    <div className="actions-row" style={{ marginTop: 0 }}>
                      <button type="button" className="btn btn-secondary" onClick={swapFirstLastForAllRows} disabled={isUpdating || editorRows.length === 0}>
                        Swap first / last
                      </button>
